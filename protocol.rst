...
The TermySequence Protocol defines the interface to the :doc:`termy-server <server>` multiplexer server. A `design goal <https://termysequence.io/posts/first-post.html>`_ of the protocol is to make it as easy as possible to write new TermySequence clients. A :ref:`minimal client <strategy-minimal>` which only displays terminal screen contents can ignore most of the protocol entirely. Features such as :ref:`scrollback <strategy-scrollback>`, history, annotations, and file system support can be added to the client later if desired.
